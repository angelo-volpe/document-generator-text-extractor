...
def train_model(model, train_data_loader, val_data_loader, optimizer, device="cpu", num_epochs=1):
    """
    ...
    for epoch in range(num_epochs):
        ...
        for images, targets in tqdm(train_data_loader):
            ...
            epoch_loss_dict = {k: epoch_loss_dict.get(k, 0) + loss_dict.get(k, 0) for k in list(loss_dict)}
        ...
        epoch_loss_dict = {k: v.item()/len(train_data_loader) for k, v in epoch_loss_dict.items()}
        # Print epoch loss
        ...
        [_, _], [precision, recall] = evaluate_model(model, val_data_loader, device, prediction_threshold=0.5, iou_thresold=0.95)
